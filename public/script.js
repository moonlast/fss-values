    ...
    const TRADE_ADS = [
        {
            ...
            username: "WM5M",
            posterId: "admin",
            pfp: "https://cdn.discordapp.com/avatars/896367467834789938/5bfdb20b5aad84ab655fd756013ac0ef.png",
            offering: [
                ...
                { name: "WC26 Backwards Celebration", qty: 1, image: "https://tr.rbxcdn.com/180DAY-978cff2d7d7ab20ce9287b76a3c8be24/420/420/Image/Webp/noFilter", value: "125K" },
                ...
                { name: "Gold Champion Band [B]", qty: 1, image: "https://raw.githubusercontent.com/moonlast/fss-values/refs/heads/main/goldb.png", value: "120K" },
            ...
            looking_for: "offers",
            notes: "Message me on discord: @wm5m",
            timestamp: new Date("2026-08-28T10:30:00").getTime(),
            offeringTotal: 0,
            lookingTotal: 0
        },
        {
            ...
            posterId: "admin",
            pfp: "https://cdn.discordapp.com/avatars/635129135865266178/571f75d7532b3b7b8d42f22b1dff3a24.png",
            offering: [
    ...
    // This page does not contain the values grids
    if (!tier4Grid || !highGrid || !midGrid || !lowGrid) {
        return;
    }
    ...
    function openItemPicker(side) {
        ...
        if (modal) {
            ...
            if (document.getElementById('createAdModal')?.classList.contains('active')) {
                renderAdModalItems('');
            } else {
                renderModalItems('');
            }
    ...
    function renderModalItems(search = '') {
        ...
        filtered.sort((a, b) => b.numericValue - a.numericValue);
    ...
    let currentAdPickerSide = 'offering';

    function getAllItemsForAdPicker() {
        const items = [];
        ITEMS.forEach(item => {
            if (item.isDual) {
                item.items.forEach(sub => {
    ...
    function openAdItemPicker(side) {
        currentAdPickerSide = side;
        const modal = document.getElementById('itemPickerModal');
        if (modal) {
            modal.classList.add('active');
            renderAdModalItems('');
    ...
    function renderAdModalItems(search = '') {
        const grid = document.getElementById('modalGrid');
        if (!grid) return;
        grid.innerHTML = '';
        
        const allItems = getAllItemsForAdPicker();
        const searchLower = search.toLowerCase().trim();
        ...
        filtered.forEach(item => {
            ...
            div.onclick = function() {
                addItemToAd(currentAdPickerSide, item);
            };
    ...
    function addItemToAd(side, item) {
        const existing = adItems[side].find(i => i.name === item.name);
        if (existing) {
            existing.qty = (existing.qty || 1) + 1;
        } else {
            adItems[side].push({ ...item, qty: 1 });
        }
        closeItemPicker();
        renderAdItemsList();
    }

    function removeItemFromAd(side, index) {
        adItems[side].splice(index, 1);
        renderAdItemsList();
    }

    function renderAdItemsList() {
        const containers = {
            offering: document.getElementById('adOfferingItems'),
            looking_for: document.getElementById('adLookingForItems')
        };

        ['offering', 'looking_for'].forEach(side => {
            const container = containers[side];
            if (!container) return;

            container.innerHTML = '';

            if (adItems[side].length === 0) {
                const empty = document.createElement('p');
                empty.className = 'empty-msg';
                empty.textContent = 'No items added';
                container.appendChild(empty);
            } else {
                adItems[side].forEach((item, index) => {
                    const div = document.createElement('div');
                    ...
                    const tooltip = document.createElement('div');
                    tooltip.className = 'ad-item-tooltip';
                    tooltip.textContent = `${item.name} · ${item.value}`;
                    div.appendChild(tooltip);

                    if (item.qty > 1) {
                        const badge = document.createElement('span');
                        badge.className = 'ad-item-qty';
                        badge.textContent = item.qty;
                        div.appendChild(badge);
                    }

                    const removeBtn = document.createElement('button');
                    removeBtn.className = 'ad-item-remove';
                    removeBtn.textContent = '✕';
                    removeBtn.onclick = (e) => {
                        e.stopPropagation();
                        removeItemFromAd(side, index);
                    };
    ...
    function renderTradeAds() {
        ...
        // Use ONLY the hardcoded TRADE_ADS array
        let ads = [...TRADE_ADS];

        // Calculate totals for each ad
        ads.forEach(ad => {
            ad.offeringTotal = ad.offering.reduce((total, adItem) => {
                const item = ITEMS.find(
                    i => i.name.toLowerCase() === adItem.name.toLowerCase()
                );
                return total + ((item?.numericValue || 0) * (adItem.qty || 1));
            }, 0);

            if (ad.looking_for === "offers") {
                ad.lookingTotal = 0;
            } else if (Array.isArray(ad.looking_for)) {
                ad.lookingTotal = ad.looking_for.reduce((total, adItem) => {
                    const item = ITEMS.find(
                        i => i.name.toLowerCase() === adItem.name.toLowerCase()
                    );
                    return total + ((item?.numericValue || 0) * (adItem.qty || 1));
        ...
        if (search) {
            ads = ads.filter(ad => {
                ...
                let lookingMatch = false;
                if (ad.looking_for === "offers") {
                    lookingMatch = "offers".includes(search);
                } else if (Array.isArray(ad.looking_for)) {
                    lookingMatch = ad.looking_for.some(i => i.name.toLowerCase().includes(search));
                }
                return usernameMatch || offeringMatch || lookingMatch;
            });
        }
        ...
        ads.forEach(ad => {
            ...
            if (ad.looking_for === "offers") {
                ...
                `;
            } else if (Array.isArray(ad.looking_for) && ad.looking_for.length > 0) {
                lookingHtml = `
                    <div class="ad-label">🔵 Looking For:</div>
                    <div class="ad-items-grid-display">
                        ${ad.looking_for.map(i => `
                            <div class="ad-item-display">
                                <img src="${i.image || ''}" alt="${i.name}" loading="lazy">
                                ${i.qty > 1 ? `<span class="ad-item-qty-display">${i.qty}</span>` : ''}
                                <div class="ad-item-name-display">${i.name}</div>
    ...
    <div class="user-icon">
        ${
            ad.pfp
                ? `<img src="${ad.pfp}" alt="${ad.username}" loading="lazy">`
                : '👤'
        }
    </div>
    ${ad.username}
                ...
                <div class="ad-value-total">
                    Offering: ${formatValue(ad.offeringTotal)}
                    · Looking For: ${formatValue(ad.lookingTotal)}
                    ${ad.offeringTotal > 0 && ad.lookingTotal > 0 ? ` · ${ad.offeringTotal > ad.lookingTotal ? '🔴 Overpay' : ad.offeringTotal < ad.lookingTotal ? '🟢 Underpay' : '⚖️ Fair'}` : ''}
                </div>
    ...
    function viewAd(id) {
        ...
        ${
            ...
            ` : ''}
            ${ad.looking_for !== "offers" && ad.looking_for.length > 0 ? `
                <div class="ad-detail-items">
                    <div class="side-label">🔵 Looking For:</div>
                    <div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:4px;">
                        ${ad.looking_for.map(i => `
                            <div style="display:flex;flex-direction:column;align-items:center;gap:2px;">
                                <img src="${i.image || ''}" alt="${i.name}" style="width:48px;height:48px;object-fit:contain;border-radius:8px;border:1px solid var(--border-card);padding:4px;background:var(--bg-body);">
                                <div style="font-size:0.65rem;font-weight:700;color:var(--text-secondary);text-align:center;">${i.name}${i.qty > 1 ? ` ×${i.qty}` : ''}</div>
                            </div>
                        `).join('')}
                    </div>
                </div>
            ` : ad.looking_for === "offers" ? `
                <div class="ad-detail-items">
                    <div class="side-label">🔵 Looking For:</div>
            ...
            <div class="ad-detail-total">
                Offering: ${formatValue(ad.offeringTotal)} · Looking For: ${formatValue(ad.lookingTotal)}
            </div>
            ${ad.notes ? `<div class="ad-detail-notes">📝 ${ad.notes}</div>` : ''}
            <div class="ad-detail-roblox">
                🎮 Roblox Username: <a href="https://www.roblox.com/search/users?keyword=${encodeURIComponent(ad.username)}" target="_blank">${ad.username}</a>
            </div>
    ...
    function init() {
    // Values page
    if (document.getElementById('tier4Grid')) {
        renderItems('all', 'all', '');
    }
    ...
    if (yourTokens) {
        yourTokens.addEventListener('input', updateCalcTotals);
    }

    if (theirTokens) {
        theirTokens.addEventListener('input', updateCalcTotals);
    }
